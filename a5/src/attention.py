...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        B, T, C = x.size()  # l = T, d = C

        l1 = F.relu(self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2))  # (B, nh, T, hs)
        l2 = l1 @ self.w2[:, :T] + self.b2[:T]  # (B, nh, T, T)
        l2 = l2.masked_fill(self.mask[:, :, :T, :T] == 0, -1e10)
        l2 = F.softmax(l2, dim=-1)  # (B, nh, T, T)
        l2 = self.attn_drop(l2)
        lv = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2)  # (B, nh, T, hs)
        y = l2 @ lv  # (B, nh, T, hs)
        y = y.transpose(1, 2).contiguous().view(B, T, C)  # re-assemble all head outputs side by side
