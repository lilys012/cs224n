...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        trunc_len = random.randint(min(4, len(document)), min(int(self.block_size*7/8), len(document)))
        #trunc_start = random.randint(0, len(document) - trunc_len)
        trunc_document = document[:trunc_len]
        randnum = random.randint(1, 4)
        mask_len = int(random.random()*trunc_len/4) if randnum > 1 else int(trunc_len/4 + random.random()*3*trunc_len/4)
        # n/8 * a + 5n/8 * (1-a) = 2n/8 -> a+5-5a=2, a = 3/4
        ...
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.MASK_CHAR + self.PAD_CHAR*(self.block_size - 3 - trunc_len)
        x = masked_string[:-1]
        y = masked_string[1:]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
