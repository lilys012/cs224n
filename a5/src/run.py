...
mconf = model.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,
    n_layer=4, n_head=8, n_embd=256, variant=args.variant)
...
if args.function == 'pretrain':
    ...
    #     num_workers=4
    pretconf = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3, lr_decay=True,
                                  warmup_tokens=512 * 20,
                                  final_tokens=200 * len(pretrain_dataset) * block_size, num_workers=4)
    pre_trainer = trainer.Trainer(model, pretrain_dataset, None, pretconf)
    pre_trainer.train()
    ...
    #         final_tokens=200*len(pretrain_dataset)*block_size
    #         num_workers=4
    finetune_corpus = dataset.NameDataset(pretrain_dataset, open(args.finetune_corpus_path).read())
    if args.reading_params_path is not None:
        ...
        tconf = trainer.TrainerConfig(max_epochs=75, batch_size=256, learning_rate=6e-4, lr_decay=True,
                                      warmup_tokens=512 * 20,
                                      final_tokens=200 * len(pretrain_dataset) * block_size, num_workers=4)
    fine_trainer = trainer.Trainer(model, finetune_corpus, None, tconf)
    fine_trainer.train()
    torch.save(model.state_dict(), args.writing_params_path)
    #raise NotImplementedError
