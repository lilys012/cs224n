...
def naiveSoftmaxLossAndGradient(
    ...
    sfm = softmax(outsideVectors.dot(centerWordVec))  # (num words in vocab, )
    loss = -np.log(sfm[outsideWordIdx])
    sfm[outsideWordIdx] -= 1
    gradCenterVec = outsideVectors.T.dot(sfm)
    gradOutsideVecs = sfm.reshape(sfm.shape[0], -1).dot(centerWordVec.reshape(-1, centerWordVec.shape[0]))
...
def negSamplingLossAndGradient(
    ...
    mysample = outsideVectors[negSampleWordIndices]  # (k, word length)
    loss -= np.sum(np.log(sigmoid(-mysample.dot(centerWordVec))))  # (k, 1)
    gradCenterVec = np.sum(mysample * (1-sigmoid(-mysample.dot(centerWordVec))).reshape(K, -1), axis=0)
    gradCenterVec -= outsideVectors[outsideWordIdx] * (1-sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec)))
    mask = np.zeros((outsideVectors.shape[0], 1))
    for idx in negSampleWordIndices:
        mask[idx] += 1
    out_masked = outsideVectors * (mask > 0)  # (n, word_length)
    gradOutsideVecs = (1-sigmoid(-out_masked.dot(centerWordVec))).reshape(-1, 1).dot(centerWordVec.reshape(1, -1)) * mask
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        temploss, tempGradCV, tempGradOV = word2vecLossAndGradient(centerWordVec, word2Ind[word], outsideVectors, dataset)
        loss += temploss
        gradCenterVecs[word2Ind[currentCenterWord]] += tempGradCV
        gradOutsideVectors += tempGradOV
