...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        b = enc_hiddens.shape[0]
        init_decoder_hidden = self.h_projection(torch.permute(last_hidden, (1, 0, 2)).reshape(b, -1))
        init_decoder_cell = self.c_projection(torch.permute(last_cell, (1, 0, 2)).reshape(b, -1))
